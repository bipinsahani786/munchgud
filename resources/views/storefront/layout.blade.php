...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Outfit:wght@300;400;500;600;700;800;900&family=Playfair+Display:ital,wght@0,400;0,600;0,700;0,900;1,400;1,700&family=DM+Sans:ital,opsz,wght@0,9..40,300;0,9..40,400;0,9..40,500;0,9..40,600;1,9..40,400&display=swap" rel="stylesheet">
    ...
    <style>
        :root {
            --primary: {{ $theme->primary_color ?? '#2B6E2F' }};
            --primary-dark: {{ $theme->primary_color ?? '#1B4332' }};
            --secondary: {{ $theme->secondary_color ?? '#E07B2A' }};
            --bg: {{ $theme->bg_color ?? '#FAFAF5' }};
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; }
        ...
        ::-webkit-scrollbar-thumb { background: #2B6E2F; border-radius: 99px; }
        ...
        #mainNav { transition: background 0.4s ease, box-shadow 0.4s ease, border-color 0.4s ease; }
        #mainNav.scrolled { background: rgba(250,250,245,0.96) !important; backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); box-shadow: 0 2px 40px rgba(27,67,50,0.09); }

        /* ─── Dropdown ───────────────────────────────── */
        [x-cloak] { display: none !important; }
        .dropdown-enter { animation: dropIn 0.22s cubic-bezier(0.16,1,0.3,1) forwards; }
        @keyframes dropIn { from { opacity: 0; transform: translateY(-8px) scale(0.98); } to { opacity: 1; transform: translateY(0) scale(1); } }

        /* ─── Mobile Drawer ──────────────────────────── */
        .drawer-slide { transition: transform 0.38s cubic-bezier(0.16,1,0.3,1), opacity 0.3s ease; }
        .drawer-overlay { transition: opacity 0.3s ease; }

        /* ─── Footer ─────────────────────────────────── */
        .footer-link { transition: color 0.2s, padding-left 0.2s; }
        .footer-link:hover { color: #66BB6A; padding-left: 4px; }
        ...
        /* ─── Counter ────────────────────────────────── */
        .parallax-hero { transition: transform 0.1s linear; will-change: transform; }

        /* ─── Buttons ────────────────────────────────── */
        .btn-primary {
            display: inline-flex; align-items: center; justify-content: center; gap: 8px;
            background: #2B6E2F; color: #fff; font-family: 'Outfit', sans-serif;
        ...
        .btn-primary:hover { background: #1B4332; transform: scale(1.03); box-shadow: 0 8px 28px rgba(43,110,47,0.32); }
        .btn-primary:active { transform: scale(0.98); }

        .btn-outline {
            display: inline-flex; align-items: center; justify-content: center; gap: 8px;
            background: transparent; color: #1A1A1A; font-family: 'Outfit', sans-serif;
            font-weight: 600; font-size: 13px; letter-spacing: 0.05em;
            padding: 11px 28px; border-radius: 999px; border: 1.5px solid rgba(27,67,50,0.18);
            transition: border-color 0.25s, color 0.25s, background 0.25s;
        }
        .btn-outline:hover { border-color: #2B6E2F; color: #2B6E2F; background: rgba(43,110,47,0.04); }
        ...
        /* ─── Premium shadow ─────────────────────────── */
        .premium-shadow { box-shadow: 0 4px 40px rgba(27,67,50,0.08), 0 1px 4px rgba(0,0,0,0.04); }

        /* ─── Toast ──────────────────────────────────── */
        @keyframes slideInToast { from { transform: translateX(110%); opacity: 0; } to { transform: translateX(0); opacity: 1; } }
        .toast-in { animation: slideInToast 0.4s cubic-bezier(0.16,1,0.3,1) forwards; }
    </style>
    @yield('styles')
</head>
...
<div class="bg-mg-green-dark text-white relative overflow-hidden" 
     x-data="{ msgs: ['🌿 Free Shipping on orders above ₹999 — Pan India Delivery!', '✨ New customers get 10% off — Use code WELCOME10', '🔥 New Flavour Drop: Cheese & Herbs Makhana is LIVE!', '⭐ 5,000+ Happy Snackers — Join the MunchGud family today!'], i: 0 }" 
...
{{-- ══════════════════════════════════════════
     NAVBAR
     ══════════════════════════════════════════ --}}
<header id="mainNav" class="sticky top-0 z-[999] bg-mg-cream border-b border-black/[0.04]" 
        x-data="{ 
            mobileOpen: false, 
            shopOpen: false, 
            searchOpen: false,
        ...
        <div class="flex items-center justify-between h-[72px] lg:h-[84px] gap-8">
            ...
            <a href="/" class="flex-shrink-0 group">
                @if(!empty($global_settings['company_logo']))
                    <img src="{{ Storage::url($global_settings['company_logo']) }}" alt="{{ $global_settings['company_name'] ?? 'MunchGud' }}" class="h-10 lg:h-[56px] w-auto object-contain transition-transform duration-300 group-hover:scale-105 rounded-md">
                @else
                    <img src="/images/logo.jpg" alt="MunchGud" class="h-10 lg:h-[56px] w-auto object-contain transition-transform duration-300 group-hover:scale-105 rounded-md">
                @endif
            </a>
            ...
            <div class="flex items-center gap-1 ml-auto">
                ...
                <button @click="searchOpen = true" aria-label="Search"
                    ...
                    <svg width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35" stroke-linecap="round"/></svg>
                </button>
                ...
                <a href="{{ route('account.wishlist') }}" aria-label="Wishlist"
                   ...
                   @wishlist-updated.window="count = $event.detail.count">
                    <svg width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z" stroke-linecap="round" stroke-linejoin="round"/></svg>
                    ...
                    <span x-show="count > 0" x-text="count" x-cloak
                          ...
                          x-transition:leave="transition scale-100"
                          x-transition:leave-start="scale-100"
                          x-transition:leave-end="scale-0"></span>
                </a>
                ...
                @auth
                    <div class="relative hidden sm:block" x-data="{ open: false }" @click.away="open = false">
                        <button @click="open = !open" class="flex items-center gap-1.5 p-1.5 pr-3 rounded-full hover:bg-mg-green/[0.06] border border-transparent hover:border-mg-green/10 transition-all duration-200">
                            <div class="w-7 h-7 rounded-full bg-mg-green text-white flex items-center justify-center font-bold text-[11px] uppercase shadow-inner">
                                {{ substr(auth()->user()->name, 0, 1) }}
                            </div>
                            <span class="text-[13px] font-bold text-mg-dark truncate max-w-[80px]">{{ explode(' ', auth()->user()->name)[0] }}</span>
                            <svg class="w-3 h-3 text-mg-muted transition-transform duration-200" :class="{'rotate-180': open}" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/></svg>
                        </button>
                        
                        <div x-show="open" x-cloak x-transition class="absolute right-0 mt-2 w-48 bg-white border border-mg-dark/5 shadow-xl shadow-mg-dark/5 rounded-2xl py-2 z-50 premium-shadow">
                            <a href="{{ route('account.index') }}" class="block px-4 py-2.5 text-[13px] text-mg-dark hover:bg-mg-green/5 hover:text-mg-green font-bold transition">My Account</a>
                            <a href="{{ route('account.orders') }}" class="block px-4 py-2.5 text-[13px] text-mg-dark hover:bg-mg-green/5 hover:text-mg-green font-bold transition">Orders</a>
                            <form action="{{ route('logout') }}" method="POST" class="border-t border-mg-dark/5 mt-1 pt-1">
                                @csrf
                                <button type="submit" class="w-full text-left px-4 py-2.5 text-[13px] text-red-500 hover:bg-red-50 font-bold transition">Sign Out</button>
                            </form>
                        </div>
                    </div>
                @else
                    <a href="{{ route('login') }}" class="hidden sm:flex items-center gap-1.5 px-4 py-2 rounded-xl text-[13px] font-bold text-mg-dark bg-mg-green/5 hover:bg-mg-green hover:text-white transition-all duration-200 ml-1">
                ...
                <a href="{{ route('products.index') }}" class="hidden lg:inline-flex btn-primary text-xs ml-2 py-2.5 px-5">
                    Shop Now
                    <svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M5 12h14m-7-7 7 7-7 7" stroke-linecap="round" stroke-linejoin="round"/></svg>
                </a>
                ...
                <button @click="mobileOpen = true" aria-label="Open menu"
                    ...
                    <svg width="22" height="22" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" d="M4 6h16M4 12h16M4 18h10"/></svg>
                </button>
            </div>
        </div>
    </div>
    ...
    <div x-show="mobileOpen" x-cloak class="fixed inset-0 z-[1001] lg:hidden" @keydown.escape.window="mobileOpen = false">
        <!-- Backdrop -->
        <div class="absolute inset-0 bg-black/60 backdrop-blur-sm drawer-overlay"
             x-show="mobileOpen"
        ...
        <!-- Drawer Panel -->
        <div class="absolute right-0 top-0 bottom-0 w-[82vw] max-w-sm bg-white shadow-2xl flex flex-col"
             x-show="mobileOpen"
            ...
            <!-- Drawer Header -->
            <div class="flex items-center justify-between px-5 py-4 border-b border-black/[0.06] bg-mg-cream">
                <a href="/" @click="mobileOpen = false">
                    @if(!empty($global_settings['company_logo']))
                        <img src="{{ Storage::url($global_settings['company_logo']) }}" alt="{{ $global_settings['company_name'] ?? 'MunchGud' }}" class="h-9 w-auto object-contain rounded">
                    @else
                        <img src="/images/logo.jpg" alt="MunchGud" class="h-9 w-auto object-contain rounded">
                    @endif
                </a>
                <button @click="mobileOpen = false" class="p-2 rounded-xl hover:bg-black/5 transition text-mg-dark/60">
                    <svg width="20" height="20" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>
            </div>

            <!-- Drawer Navigation -->
            <div class="flex-1 overflow-y-auto py-4 px-3">
                <a href="/" @click="mobileOpen=false" class="flex items-center gap-3 px-4 py-3.5 rounded-xl hover:bg-mg-green/[0.05] text-mg-dark font-semibold text-[14.5px] transition {{ request()->is('/') ? 'bg-mg-green/[0.06] text-mg-green' : '' }}">
                    <svg class="w-[18px] h-[18px] text-mg-green/70" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
                    Home
                </a>

                <!-- Shop Accordion -->
                <div x-data="{ shopOpen: false }">
                    <button @click="shopOpen = !shopOpen" class="w-full flex items-center justify-between px-4 py-3.5 rounded-xl hover:bg-mg-green/[0.05] text-mg-dark font-semibold text-[14.5px] transition">
                        <span class="flex items-center gap-3">
                            <svg class="w-[18px] h-[18px] text-mg-green/70" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" stroke-linecap="round" stroke-linejoin="round"/></svg>
                            Shop
                        </span>
                        <svg class="w-4 h-4 text-mg-muted transition-transform duration-250" :class="shopOpen ? 'rotate-180' : ''" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M19 9l-7 7-7-7" stroke-linecap="round" stroke-linejoin="round"/></svg>
                    </button>
                    <div x-show="shopOpen" x-collapse x-cloak class="ml-4 pl-4 border-l-2 border-mg-green/15 mt-1 mb-2 space-y-0.5">
                        <a href="{{ route('products.index') }}" @click="mobileOpen=false" class="block py-2.5 px-3 text-sm text-mg-dark/70 font-medium hover:text-mg-green rounded-lg hover:bg-mg-green/[0.04] transition">All Products</a>
                        @if(isset($global_categories))
                            @foreach($global_categories as $cat)
                            <a href="{{ route('products.category', $cat->slug) }}" @click="mobileOpen=false" class="block py-2.5 px-3 text-sm text-mg-dark/70 font-medium hover:text-mg-green rounded-lg hover:bg-mg-green/[0.04] transition">{{ $cat->name }}</a>
                            @endforeach
                        @endif
                        <a href="{{ route('build-a-box') }}" @click="mobileOpen=false" class="flex items-center gap-2 py-2.5 px-3 text-sm font-semibold text-mg-orange hover:bg-mg-orange/5 rounded-lg transition">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M20 7H4a2 2 0 00-2 2v10a2 2 0 002 2h16a2 2 0 002-2V9a2 2 0 00-2-2z"/><path d="M16 21V5a2 2 0 00-2-2h-4a2 2 0 00-2 2v16"/></svg>
                            Build a Box
                        </a>
                    </div>
                </div>
                ...
                <div class="border-t border-black/[0.05] mt-3 pt-3">
                    @auth
                        <a href="{{ route('account.index') }}" @click="mobileOpen=false" class="flex items-center gap-3 px-4 py-3.5 rounded-xl hover:bg-mg-green/[0.05] text-mg-dark font-semibold text-[14.5px] transition">
                            <svg class="w-[18px] h-[18px] text-mg-green/70" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                            My Account
                        </a>
                        <a href="{{ route('account.orders') }}" @click="mobileOpen=false" class="flex items-center gap-3 px-4 py-3.5 rounded-xl hover:bg-mg-green/[0.05] text-mg-dark font-semibold text-[14.5px] transition">
                            <svg class="w-[18px] h-[18px] text-mg-green/70" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" stroke-linecap="round" stroke-linejoin="round"/></svg>
                            Track Order
                        </a>
                        <form action="{{ route('logout') }}" method="POST">
                            @csrf
                            <button type="submit" class="w-full flex items-center gap-3 px-4 py-3.5 rounded-xl hover:bg-red-50 text-red-500 font-semibold text-[14.5px] transition">
                    ...
                    @else
                        <a href="{{ route('login') }}" @click="mobileOpen=false" class="flex items-center gap-3 px-4 py-3.5 rounded-xl hover:bg-mg-green/[0.05] text-mg-green font-bold text-[14.5px] transition bg-mg-green/5">
                            <svg class="w-[18px] h-[18px]" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6a2.25 2.25 0 00-2.25 2.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15m3 0l3-3m0 0l-3-3m3 3H9"/></svg>
                            Log In / Register
                        </a>
                    @endauth
                </div>
            ...
            <!-- Drawer Footer CTA -->
            <div class="p-4 border-t border-black/[0.06] bg-mg-cream">
                <a href="{{ route('products.index') }}" @click="mobileOpen=false" class="btn-primary w-full text-sm py-3.5">
                    Shop Now — From ₹199
                </a>
                <p class="text-center text-[11px] text-mg-muted mt-2">Free shipping above ₹999 ✦ Pan India</p>
            </div>
        </div>
    </div>
</header>
...
<footer class="bg-[#0F1F16] text-white/60 relative overflow-hidden">
    ...
    <div class="absolute inset-0 pointer-events-none opacity-[0.03]" style="background-image: radial-gradient(circle at 20% 80%, #2B6E2F 0%, transparent 50%), radial-gradient(circle at 80% 20%, #E07B2A 0%, transparent 40%);"></div>
    ...
    <div class="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-10 py-16">
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-10 lg:gap-8">
            ...
            <div class="col-span-1 sm:col-span-2 lg:col-span-2">
                <a href="/">
                    @if(!empty($global_settings['company_logo']))
                        <img src="{{ Storage::url($global_settings['company_logo']) }}" alt="{{ $global_settings['company_name'] ?? 'MunchGud' }}" class="h-11 w-auto object-contain rounded-md mb-5 brightness-125">
                    @else
                        <img src="/images/logo.jpg" alt="MunchGud" class="h-11 w-auto object-contain rounded-md mb-5 brightness-125">
                    @endif
                </a>
                <p class="text-white/35 text-sm leading-relaxed max-w-[240px] mb-6">
                    ...
                    @if(!empty($global_settings['company_pincode'])) {{ $global_settings['company_pincode'] }} @endif
                </p>

                <!-- Social Links with actual SVGs -->
                <div class="flex gap-2.5">
                    @if(!empty($global_settings['social_instagram']))
                    <a href="{{ $global_settings['social_instagram'] }}" target="_blank" aria-label="Instagram" class="w-9 h-9 rounded-xl bg-white/[0.06] border border-white/[0.07] flex items-center justify-center text-white/40 hover:bg-[#E1306C] hover:text-white hover:border-transparent transition-all duration-200">
                    ...
                    <a href="{{ $global_settings['social_facebook'] }}" target="_blank" aria-label="Facebook" class="w-9 h-9 rounded-xl bg-white/[0.06] border border-white/[0.07] flex items-center justify-center text-white/40 hover:bg-[#1877F2] hover:text-white hover:border-transparent transition-all duration-200">
                    ...
                    <a href="{{ $global_settings['social_twitter'] }}" target="_blank" aria-label="X (Twitter)" class="w-9 h-9 rounded-xl bg-white/[0.06] border border-white/[0.07] flex items-center justify-center text-white/40 hover:bg-black hover:text-white hover:border-transparent transition-all duration-200">
                        <svg width="14" height="14" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.742l7.73-8.835L1.254 2.25H8.08l4.253 5.622zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                    </a>
                    @endif
            ...
            <div>
                ...
                <ul class="space-y-3">
                    <li><a href="{{ route('products.index') }}" class="footer-link text-sm text-white/40 hover:text-mg-leaf inline-block">All Products</a></li>
                    <li><a href="{{ route('products.index') }}?sort=bestsellers" class="footer-link text-sm text-white/40 hover:text-mg-leaf inline-block">Bestsellers</a></li>
                    <li><a href="{{ route('products.index') }}?sort=new" class="footer-link text-sm text-white/40 hover:text-mg-leaf inline-block">New Arrivals</a></li>
                    <li><a href="{{ route('build-a-box') }}" class="footer-link text-sm text-white/40 hover:text-mg-leaf inline-block">Build a Box</a></li>
                    @if(isset($global_categories))
                        @foreach($global_categories->take(3) as $cat)
                        <li><a href="{{ route('products.category', $cat->slug) }}" class="footer-link text-sm text-white/40 hover:text-mg-leaf inline-block">{{ $cat->name }}</a></li>
                        @endforeach
                    @endif
                </ul>
            </div>
            ...
            <div>
                ...
                <ul class="space-y-3">
                    ...
                    <li><a href="{{ route('about') }}" class="footer-link text-sm text-white/40 hover:text-mg-leaf inline-block">About Us</a></li>
                </ul>
            </div>
            ...
            <div>
                ...
                <ul class="space-y-3">
                    <li><a href="{{ route('faq') }}" class="footer-link text-sm text-white/40 hover:text-mg-leaf inline-block">FAQ</a></li>
                    <li><a href="{{ route('contact') }}" class="footer-link text-sm text-white/40 hover:text-mg-leaf inline-block">Contact Us</a></li>
                    <li><a href="{{ route('account.orders') }}" class="footer-link text-sm text-white/40 hover:text-mg-leaf inline-block">Track Order</a></li>
                    <li><a href="{{ route('shipping') }}" class="footer-link text-sm text-white/40 hover:text-mg-leaf inline-block">Shipping Policy</a></li>
                    <li><a href="{{ route('refund') }}" class="footer-link text-sm text-white/40 hover:text-mg-leaf inline-block">Returns & Refunds</a></li>
                </ul>
            </div>
            ...
            <div>
                ...
                <ul class="space-y-4">
                    <li class="flex items-start gap-2.5">
                        <svg class="w-4 h-4 text-mg-leaf mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                        <a href="mailto:{{ $global_settings['store_email'] ?? 'support@munchgud.com' }}" class="text-sm text-white/40 hover:text-mg-leaf transition">{{ $global_settings['store_email'] ?? 'support@munchgud.com' }}</a>
                    </li>
                    <li class="flex items-start gap-2.5">
                        ...
                        <span class="text-sm text-white/35 leading-snug">{{ $global_settings['company_city'] ?? 'Darbhanga' }}, {{ $global_settings['company_state'] ?? 'Bihar' }}<br>India — {{ $global_settings['company_pincode'] ?? '846 004' }}</span>
                    </li>
                    <li>
                        <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $global_settings['store_phone'] ?? '919999999999') }}" target="_blank" rel="noopener" class="inline-flex items-center gap-2 text-xs font-semibold bg-[#25D366]/10 text-[#25D366] border border-[#25D366]/20 rounded-full px-3.5 py-2 hover:bg-[#25D366]/20 transition">
    ...
    <div class="border-t border-white/[0.05]">
        <div class="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-10 py-6 pb-24 sm:pb-6 flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div class="flex flex-wrap items-center justify-center gap-3 sm:gap-4 mt-4 sm:mt-0">
                <a href="{{ route('privacy') }}" class="text-[11.5px] text-white/30 hover:text-white/60 transition">Privacy</a>
                <span class="text-white/15 hidden sm:inline">·</span>
                <a href="{{ route('terms') }}" class="text-[11.5px] text-white/30 hover:text-white/60 transition">Terms</a>
                <span class="text-white/15 hidden sm:inline">·</span>
                <div class="flex flex-wrap items-center justify-center gap-1.5 w-full sm:w-auto sm:pl-2 mt-2 sm:mt-0">
                    <span class="text-[10px] text-white/20 uppercase tracking-wider w-full sm:w-auto text-center sm:text-left mb-1 sm:mb-0">We accept</span>
                    @foreach(['UPI', 'Visa', 'MC', 'COD', 'GPay'] as $pay)
                    <span class="bg-white/[0.06] border border-white/[0.07] text-white/30 text-[9.5px] font-bold px-2 py-0.5 rounded-md tracking-wider">{{ $pay }}</span>
                    @endforeach
                </div>
            </div>
        </div>
...
<div x-data="{ show: false, message: '', icon: '' }"
    ...
    <span x-show="icon === 'success'" class="w-6 h-6 rounded-full bg-mg-green flex items-center justify-center shrink-0">
        <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="3" viewBox="0 0 24 24"><path d="M5 13l4 4L19 7" stroke-linecap="round" stroke-linejoin="round"/></svg>
    </span>
    <span x-show="icon === 'removed'" class="w-6 h-6 rounded-full bg-mg-muted/50 flex items-center justify-center shrink-0">
        <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M18 6L6 18M6 6l12 12" stroke-linecap="round" stroke-linejoin="round"/></svg>
    </span>
