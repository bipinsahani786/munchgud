...
<div class="mb-6 flex items-center gap-4">
    ...
    <div>
        <h1 class="text-xl font-bold text-gray-900 tracking-tight">Add Category</h1>
        <p class="text-xs text-gray-500 font-medium mt-1">Create a new product category.</p>
    </div>
</div>
    ...
    <form action="{{ route('admin.categories.store') }}" method="POST" class="p-6">
        ...
        <div class="space-y-5">
            <div>
                ...
                <input type="text" name="name" value="{{ old('name') }}" class="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition" required placeholder="e.g. Snacks">
                @error('name') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>

            <div>
                <label class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Parent Category (Optional)</label>
            ...
            <div>
                ...
                <textarea name="description" rows="3" class="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition resize-none">{{ old('description') }}</textarea>
            ...
            <div class="grid grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="number" name="sort_order" value="{{ old('sort_order', 0) }}" class="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition">
                </div>
